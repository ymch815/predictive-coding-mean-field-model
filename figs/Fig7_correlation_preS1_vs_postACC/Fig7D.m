% Fig 7D: correlation between preS1 and post ACC (spontaneous)
clear all 
close all
warning off 
...
% load directly
data = load('cor_spon.mat');
xarr = data.xarr;
preS1 = data.preS1; 
postACC = data.postACC; 

% calculate data
% [xarr,recell,~] = trials('naive', 'spontaneous', 'no feedback', {'preS1','postACC'}, plot_dynamics,...
...
h = colorbar;
set(get(h,'label'),'string','z(0)','fontsize',20); 
[rho,pval]=corr(preS1,postACC);
fprintf("the correlation is: %4.2f \n p value is: %4.4f \n",rho,pval)
% % % save('cor_spon.mat','xarr','preS1','postACC')