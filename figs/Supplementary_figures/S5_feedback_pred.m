% Fig S5: same barplot as Fig10C, except with feedback. 
clear all 
close all
warning off 
...
itrmax=2.2;
rep=1;
[~,recell_wro,~] = trials('naive', 'prediction_wrong', 'feedback', ...
    {'beforeS1','duringS1'}, plot_dynamics, plot_comp_rs, itr, itrmin, itrmax, rep, noise);
[~,recell_pre,~] = trials('naive', 'prediction_precise','feedback' ,...
    {'beforeS1','duringS1'}, plot_dynamics,plot_comp_rs, itr, itrmin, itrmax, rep, noise);
wro_before = recell_wro{1}(:,1); wro_dur = recell_wro{2}(:,1);
pre_before = recell_pre{1}(:,1); pre_dur = recell_pre{2}(:,1);
bar_plot([wro_before wro_dur],[pre_before pre_dur],0.08,[0.43,0.44,0.47,0.52],...
